...
for i_episode in count(1):
    ...
    while num_steps < args.batch_size:
        ...
        for t in range(10000): # Don't infinite loop while learning
            ...
            if flag:
                #adding naive perturbations
                state = state + [0,0,0,0,0.01*np.random.randn(),0.01*np.random.randn(),0.01*np.random.randn(),0.01*np.random.randn(),0.01*np.random.randn(),0.01*np.random.randn(),0]
            mask = 1
    ...
    if early >= 5:
        ...
        torch.save(policy_net.state_dict(),"Weights_policy_naive"+str(args.eps)+args.env_name+".pt")
        torch.save(value_net.state_dict(),"Weights_value_naive"+str(args.eps)+args.env_name+".pt")        
        np.save(file="Plot_array"+str(epsilon)+"_naive.npy",arr=np.array(reward_plot))
        continue
# 
    if i_episode%100 == 0:
        print("Saving Weights")
